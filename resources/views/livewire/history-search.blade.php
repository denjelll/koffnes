<div class="font-sans" style="background-image: url('{{ asset('storage/asset/gambar/motif.png') }}'); background-size: 400px 400px; background-repeat: repeat;">
    <!-- Top Navbar -->
    <header class="w-full p-4 fixed top-0 left-0 z-10 flex items-center justify-between" style="background-color: #412f26;">
    ...
    <div class="flex pt-20 h-screen pb-20">
        <!-- Sidebar Navigation -->
        <nav id="mobile-nav" class="text-white w-55 p-7 fixed top-16 h-full z-20 hidden bg-[#76634c] md:block" style="transition: transform 0.3s ease-in-out;">
            <ul class="space-y-4">
                <li><a href="{{ url('cashier') }}" class="hover:bg-opacity-50 p-2 block rounded">Home</a></li>
                <li><a href="{{ url('cashier/dashboard') }}" class="hover:bg-opacity-50 p-2 block rounded">Order</a></li>
                <li><a href="{{ url('cashier/transaksi') }}" class="hover:bg-opacity-50 p-2 block rounded">History<br>Transaksi</a></li>
                <li><a href="{{ url('cashier/stock') }}" class="hover:bg-opacity-50 p-2 block rounded">Inventory</a></li>
                <li><a href="{{ route('koffnesstatus') }}" class="hover:bg-opacity-50 p-2 block rounded">Koffnes Status</a></li>
            </ul>
        </nav>

        <!-- Content Area -->
<div class="container p-7 w-full overflow-auto relative sm:p-4 sm:ml-4 sm:mt-20 pt-[80px]">
    <!-- Search Form -->
    <div class="w-full lg:w-[50rem] lg:translate-x-[22rem] md:translate-x-[13rem] md:w-[30rem] sm:max-w-full bg-[#d8c2aa] p-7 rounded-lg shadow-md">
        ...
        <form wire:submit.prevent="search" class="flex flex-col gap-4">
            ...
            <div class="flex flex-col sm:flex-row sm:gap-2">
                <div class="flex-1">
                    ...
                    <input type="date" id="start-date" wire:model="startDate" class="w-full p-2 bg-white border border-gray-300 rounded-lg text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#412f26]">
                </div>
                <div class="flex-1 mt-2 sm:mt-0">
                    <label for="end-date" class="block text-sm font-medium text-[#412f26]">End Date</label>
                    <input type="date" id="end-date" wire:model="endDate" class="w-full p-2 bg-white border border-gray-300 rounded-lg text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#412f26]">
                </div>
            </div>
            <!-- Customer Name -->
            <div>
                <label for="customer-name" class="block text-sm font-medium text-[#412f26]">Customer Name</label>
                <input type="text" id="customer-name" wire:model="customerName" placeholder="Enter customer name" class="w-full p-2 bg-white border border-gray-300 rounded-lg text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#412f26]">
            </div>
            <!-- Submit Button -->
            <button type="submit" class="w-full py-2 bg-[#412f26] text-white rounded-lg hover:bg-[#5a4533]">Search</button>
        </form>
        ...
        <div class="mt-6">
            <div class="flex justify-between items-center mb-3">
                ...
                <form action="{{ route('daily.report') }}" method="POST">
                    @csrf
                    <button type="submit" class="bg-[#412f26] text-white py-1 px-3 rounded-lg text-sm hover:bg-[#5a4533]">Daily Report</button>
                </form>
            </div>
            <!-- Responsive Grid for Cards -->
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <h2 class="text-lg font-bold text-[#412f26] col-span-full">Total Transaction:
                    <span>Rp. {{ number_format($totalTransaction, 0, ',', '.') }}</span>
                </h2>
                @forelse ($orders as $order)
                    <li class="bg-white p-4 rounded-lg shadow-md sm:p-3 sm:text-sm">
                ...
                @empty
                    <li class="text-sm text-gray-500 col-span-full">No results found. Please refine your search.</li>
                @endforelse
            </div>
        </div>
    </div>
...
<footer class="w-full p-4 fixed bottom-0 left-0 z-30 flex flex-col items-center text-white" style="background-color: #412f26;">
        <img src="{{ asset('storage/uploads/8.png') }}" alt="Footer Logo" class="h-7 md:h-7 mb-2" style="max-width: 180px;">
        ...
        <div class="fixed inset-0 flex items-center justify-center bg-gray-600 bg-opacity-50 z-50">
            <div class="bg-[#e8d2b7] p-8 rounded-lg w-1/2 shadow-lg">
                ...
                <div class="mb-6">
                    ...
                    <p class="text-[#412f26]"><strong>Total Harga:</strong> Rp. <span>{{ number_format($showDetails['totalHarga'], 0, ',', '.') }}</span></p>
                </div>
                <div class="mt-6 space-x-4">
                    <button wire:click="$set('isShowModalOpen', false)" class="bg-[#412f26] text-white py-1 px-3 rounded hover:bg-[#5a4533]">Close</button>
                </div>
            </div>
        </div>
    @endif>
